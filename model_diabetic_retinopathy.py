...
INPUT_SIZE = (224, 224)
WEIGHTS_PATH = 'diabetic_retinopathy_disease.h5'
...
def preprocess_image(image_path: str) -> np.ndarray:
    """
    Load and preprocess a retinal fundus image for model inference.

    Steps:
        1. Resize to INPUT_SIZE (224x224 for DR model)
...
def pred_diabetic_disease(img_path: str) -> tuple[str, float]:
    """
    Predict diabetic retinopathy severity from the given image path.

    Args:
        img_path: Path to the input retinal fundus image.
    ...
    print(f"[DR] Predicted: {predicted_label} | Confidence: {confidence:.2%}")
